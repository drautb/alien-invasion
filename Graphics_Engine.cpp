...
void  cGraphics::ClearScreen(D3DCOLOR Color)
{
	// Clear The Display 
	m_D3DDevice->Clear(0, NULL, D3DCLEAR_TARGET, Color, 1.0f, 0);
}
...
bool cTexture::Render(float x, float y, RECT *SrcRect, D3DXVECTOR2 RotationCenter,
					  float RotAngleInRads, float ScaleX, float ScaleY)
{
	// Blit the image 
	HRESULT result = m_Graphics->GetSprite()->Draw(m_Texture, SrcRect, &D3DXVECTOR2(ScaleX, ScaleY), 
												   &RotationCenter, RotAngleInRads, 
					 ...
					 float ScaleX, float ScaleY)
{
	...
	// Draw Tile //
	m_Textures[TexNum].Render(DestX, DestY, &Rect, D3DXVECTOR2(0.0f,0.0f), 0.0f, ScaleX, ScaleY);
}
