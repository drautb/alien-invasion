...
// cLog class for debugging
class cLog
{
public:
	std::ofstream m_Output;

	cLog(char *Filename){m_Output.open(Filename);}
	~cLog(){m_Output.close();}

	void StartNewLine(){m_Output << std::endl;}
	void Write(const std::string &Text){m_Output << Text;}
	void Write(float FloatNumber){m_Output << FloatNumber;}
};
